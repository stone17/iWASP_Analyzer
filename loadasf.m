...
%clc
if nargin==1
    [fname,pname]=uigetfile({'*.asf;*.pit;*.asc;*.dif;*.tif;*.jpg;*.tiff;*.dat;*.mat;*.img;*.txt',...
...
if strcmp(type,'asf')
   ...
   if old==1
        ...
        fseek(fid,Pos,-1);
        1
        data(:,1)= fread(fid,inf,'1*int16',34);
        fseek(fid,Pos+2,-1);
        2
        data(:,2)= fread(fid,inf,'1*int16',34);
   end
   ...
   if old ==1
        ...
        for l=1:2
            l+2
            data(:,l+2)=fread(fid,inf,'1*float',32);
            fseek(fid,Pos+(l+1)*4,-1);
        end
   end
   if ~exist('data','var')
    data = [Col_1to2 Col_3to10];
    clear Col_1to2 Col_3to10
   end
   ...
   clear data
    ...
    %datahist(:,:)=1;
        for index=1:length(X)
            if max(round(index/length(X)*100))-[10:100]==0
                index/length(X)*100
            end
...
elseif strcmp (type,'dat') %ascci file
    ...
    incy=78;
    g=1;
elseif strcmp (type,'txt') %ascci file
    datahist=load(dateiname,'-ascii');
    options.Resize='on';
    options.WindowStyle='normal';
    options.Interpreter='tex';
	prompt = {'X increment in um:','Y increment in um:'};
	dlg_title = 'Enter pixelsize for tiff-file:';
	num_lines = 1;
	%def = {'44.3894','44.9683'};
    def = {'25','25'};
	increment =inputdlg(prompt,dlg_title,num_lines,def,options);
...
elseif strcmp (type,'iff') %tif
    ...
    %bg=double(imread([pname, 'bg_01.tiff'],'tiff'));
    %datahist=1*(2^16-datahist);
    %datahist=datahist-bg;
    dims=size(datahist);
    if dims(3)>1
        datahist=datahist(:,:,1);
    end
...
elseif strcmp (type,'mat') %mat
    if exist('lastpixel.mat','file')
        load('lastpixel.mat');
        def={num2str(incx),num2str(incy)};
    else
        def = {'25','25'};
    end
    options.Resize='on';
    ...
    options.Interpreter='tex';
	prompt = {'X increment in um:','Y increment in um:'};
	dlg_title = 'Enter pixelsize for mat-file:';
	num_lines = 1;
	%def = {'44.3894','44.9683'};
