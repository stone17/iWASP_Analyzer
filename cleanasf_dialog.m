...
if strcmp(edit,'b_spline')
	...
	set(h,'ButtonDownFcn', {@spleditorButton, handles});
    set(gcf,'DoubleBuffer','on');
	set(gcf,'KeyPressFcn',{@keypress, handles});
...
elseif strcmp(edit,'b_clip')
    ...
    try
        ...
        %splx=cell2mat(splx);
        %sply=cell2mat(sply);
        splx=getappdata(handles.clean_plot,'splx');
        sply=getappdata(handles.clean_plot,'sply');
    ...
    elseif strcmp(edit,'b_cutinv')
        ...
        setappdata(handles.clean_plot,'copy_alt',raw_old(:,1))
        setappdata(handles.clean_plot,'layers',layer_switch)
        setappdata(handles.clean_plot,'copy',paste)
        setappdata(handles.clean_plot,'mode','replot');
        min_max_Callback(hObject, eventdata, handles)
...
function [ud,splx,sply] = savesplines(handles, ax)
if nargin<2
  ax = gca;
end
...
%sply=cell2mat(sply);
%h=findobj()
%setappdata(handles.clean_plot,'splx',splx)
%setappdata(handles.clean_plot,'sply',sply)

function keypress(src,event, handles)
key = get(gcf,'CurrentCharacter');
handle_keypress(key, handles);

% Handles keypresses in window.
function handle_keypress(key, handles)
switch key
  % ESC stops editing.
 case 27,
  noedit
 ...
 case 8,
  X = getspline_X;
  if ~isempty(X)
    updatespline_X(X(:,1:end-1), handles);
 ...
 case 'o'
  zoom off
  set(gca,'ButtonDownFcn', {@spleditorButton, handles});
end
...
function spleditorButton(src,event, handles)
...
set(hk, 'Tag', 'knot', 'ButtonDownFcn',{@knotselector, size(X,2), handles}, ...
        'MarkerSize',5, 'LineWidth',4);
updatespline_X(X, handles);
...
% create handles for this spline.
...
function togglespline_closure(handles)
hs = findobj(gca,'Tag','spline');
if isempty(hs)
  warning('No current spline.');
  return
...
ud.closed = ~ud.closed;
set(hs,'UserData',ud);
Xi=renderspline(hs,ud);
setappdata(handles.clean_plot,'splx',Xi(1,:)')
setappdata(handles.clean_plot,'sply',Xi(2,:)')

function togglespline_polyline(handles)
hs = findobj(gca,'Tag','spline');
if isempty(hs)
  warning('No current spline.');
  return
...
function updatespline_X(X, handles)
hs = findobj(gca,'Tag','spline');
if isempty(hs)
  return
end
...
function knotselector(src,event,i, handles)
hknots = findobj(gca,'Tag','knotcurve');
if isempty(hknots)
  warning('Got an event on a knot, but there is no control curve.');
  return
end
set(gcf,'WindowButtonMotionFcn', {@knotSelectorMoveMouse, i,src,hknots, handles}, ...
		'WindowButtonUpFcn', @knotSelectorButtonUp);

% Drags a knot.
function knotSelectorMoveMouse(src,event,i,knot,hknotcurve, handles)
x = get(gca,'CurrentPoint');
x = [x(1,1); x(1,2)];
