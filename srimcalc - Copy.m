...
if 1==1
    ...
    %Ta
    dEdx_electron(:,3)=interp1(E_Ta,dE_electron_Ta,E);
    dEdx_ion(:,3)=interp1(E_Ta,dE_ion_Ta,E);
    range_int(:,3)=interp1(E_Ta,range_Ta,E);

    %W
    dEdx_electron(:,4)=interp1(E_W,dE_electron_W,E);
    dEdx_ion(:,4)=interp1(E_W,dE_ion_W,E);
    range_int(:,4)=interp1(E_W,range_W,E);
...
while um<=layer5end
    index=index+1;
    E_in=E_in+1;
    check=max(E_in==[1e3:1e3:100e3,110e3:10e3:1000e3,1100e3:100e3:10000e3]);
    if check==1
        if n==1
            dots='.   ';
            n=2;
    ...
    E_out=E_in;
    um=0;
    while E_out>=limit && um<=layer5end
        um=um+1;
        if um<=layer1end
        ...
        E_out=round(E_out-dEdx_electron(E_out,layerindex)+dEdx_ion(E_out,layerindex));
    end
